...
void Skin::DrawSettings()
{
	...
	ImGui::Text(T("feature.skin.advanced_skin_shader_using_dual_specular_lobes", "Advanced Skin Shader using dual specular lobes."));

	ImGui::Spacing();
	ImGui::SliderFloat(T("feature.skin.primary_roughness", "Primary Roughness"), &settings.SkinMainRoughness, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.controls_microscopic_roughness_of_stratum_corneum_layer", "Controls microscopic roughness of stratum corneum layer"));
	}

	ImGui::SliderFloat(T("feature.skin.secondary_roughness", "Secondary Roughness"), &settings.SkinSecondRoughness, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.smoothness_of_epidermal_cell_layer_reflections", "Smoothness of epidermal cell layer reflections"));
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.multiplier_for_specular_map", "Multiplier for specular map"));
		ImGui::BulletText(T("feature.skin.a_multiplier_for_the_vanilla_specular_map_applied", "A multiplier for the vanilla specular map, applied to the first layer's roughness"));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.intensity_of_secondary_specular_highlights", "Intensity of secondary specular highlights"));
	}

	ImGui::SliderFloat(T("feature.skin.fresnel_f0", "Fresnel F0"), &settings.F0, 0.0f, 0.1f, "%.4f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.fresnel_reflectance", "Fresnel reflectance"));
	}

	ImGui::SliderFloat(T("feature.skin.base_color_multiplier", "Base Color Multiplier"), &settings.BaseColorMultiplier, 0.0f, 2.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.multiplier_for_the_base_color_texture", "Multiplier for the base color texture"));
	}

	ImGui::Spacing();
	ImGui::Text(T("feature.skin.options_for_additional_roughness_and_specular_maps", "Options for additional roughness and specular maps."));
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.extra_roughness_at_the_edges_of_the_skin", "Extra roughness at the edges of the skin, to approximate peach fuzz on the face."));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.translucency_of_the_sss_transmittance_effect", "Translucency of the SSS Transmittance effect"));
	}

	ImGui::SliderFloat(T("feature.skin.sss_width", "SSS Width"), &settings.sssWidth, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.width_of_the_sss_transmittance_effect", "Width of the SSS Transmittance effect"));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.adds_a_constant_layer_of_wetness_to_all", "Adds a constant layer of wetness to all skin, making it look slightly damp or sweaty at all times, even when not in water or exerting effort."));
	}

	ImGui::SliderFloat(T("feature.skin.wetness_fade_out_time", "Wetness Fade Out Time"), &settings.WetFadeTime, 0.0f, 50.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.how_many_seconds_it_takes_for_skin_to", "How many seconds it takes for skin to fully dry after leaving water. Higher values mean wetness lingers longer."));
	}

	ImGui::SliderFloat(T("feature.skin.evaporation_rate", "Evaporation Rate"), &settings.EvaporationRate, 0.1f, 5.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.how_quickly_wetness_dries_on_different_body_parts", "How quickly wetness dries on different body parts. Higher values make the character dry faster. Works as a multiplier on Fade Out Time."));
	}

	if (isDynamicWetnessAvailable) {
		ImGui::Text(T("feature.skin.dynamic_wetness_detected", "Dynamic Wetness detected."));
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.controls_the_size_of_the_wet_dry_pattern", "Controls the size of the wet/dry pattern on skin. Higher values create a finer, more detailed pattern; lower values produce larger, broader wet patches."));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.controls_how_much_fine_detail_is_added_to", "Controls how much fine detail is added to the wetness pattern. Higher values add more small-scale variation on top of the base pattern."));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.controls_the_overall_contrast_and_roughness_of_the", "Controls the overall contrast and roughness of the wetness pattern. Higher values make the pattern more pronounced and varied."));
	}
	ImGui::SliderFloat(T("feature.skin.wetness_normal_scale", "Wetness Normal Scale"), &settings.WetParams.w, 0.0f, 20.0f, "%.1f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.controls_how_bumpy_wet_skin_appears_higher_values", "Controls how bumpy wet skin appears. Higher values create more visible surface ripples and distortion on wet areas."));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.enable_skin_detail_texture", "Enable skin detail texture"));
	}

	ImGui::SliderFloat(T("feature.skin.skin_detail_strength", "Skin Detail Strength"), &settings.SkinDetailStrength, -2.0f, 2.0f);
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.strength_of_skin_detail_texture", "Strength of skin detail texture"));
	}

	ImGui::SliderFloat(T("feature.skin.skin_detail_tiling", "Skin Detail Tiling"), &settings.SkinDetailTiling, 1.0f, 50.0f, "%1.f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.the_more_tiling_the_more_detailed_the_skin", "The more tiling, the more detailed the skin will be"));
	}
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text(T("feature.skin.multiply_the_tiling_for_the_body_to_match", "Multiply the tiling for the body to match the face"));
	}
